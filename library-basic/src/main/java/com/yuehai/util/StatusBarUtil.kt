...
import com.yuehai.util.OSUtil.isFlyme
import com.yuehai.util.OSUtil.isMiui
...
object StatusBarUtil {
     ...
     * @param window
     */
    fun setRootViewFitsSystemWindows(window: Window, fitSystemWindows: Boolean) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            val winContent = window.findViewById<View>(android.R.id.content) as ViewGroup
            if (winContent.childCount > 0) {
                val rootView = winContent.getChildAt(0) as ViewGroup
                rootView.fitsSystemWindows = fitSystemWindows
            }
     ...
     * 设置状态栏深色浅色切换
     */
    fun setStatusBarDarkTheme(window: Window, dark: Boolean): Boolean {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            when {
                Build.VERSION.SDK_INT >= Build.VERSION_CODES.M -> {
     ...
     * 设置 状态栏深色浅色切换
     */
    fun setStatusBarFontIconDark(activity: Window, @ViewType type: Int, dark: Boolean): Boolean {
        return when (type) {
            TYPE_MIUI -> setMiuiUI(activity, dark)
            TYPE_FLYME -> setFlymeUI(activity, dark)
            TYPE_M -> setCommonUI(activity, dark)
    ...
    //设置6.0 状态栏深色浅色切换
    fun setCommonUI(window: Window, dark: Boolean): Boolean {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            val decorView = window.decorView
            var vis = decorView.systemUiVisibility
            vis = if (dark) {
    ...
    //设置Flyme 状态栏深色浅色切换
    fun setFlymeUI(window: Window, dark: Boolean): Boolean {
        return try {
            val lp = window.attributes
            val darkFlag =
                WindowManager.LayoutParams::class.java.getDeclaredField("MEIZU_FLAG_DARK_STATUS_BAR_ICON")
    ...
    //设置MIUI 状态栏深色浅色切换
    fun setMiuiUI(window: Window, dark: Boolean): Boolean {
        return try {
    ...
    //        int result = UIUtil.INSTANCE.dip2px(context, 20f);
    //        try {
